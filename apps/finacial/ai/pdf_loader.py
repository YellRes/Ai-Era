...
def extract_financial_metrics(documents: list) -> dict:
    ...
    patterns = {
        # 利润表指标
        "营业收入": r"营业收入[（(]?元?[)）]?\s*[|｜]?\s*([\d,，]+\.?\d*)",
        "利润总额": r"利润总额[（(]?元?[)）]?\s*[|｜]?\s*([\d,，]+\.?\d*)",
        "归属于上市公司股东的净利润": r"归属于上市公司股东的净利润[（(]?元?[)）]?\s*[|｜]?\s*(-?[\d,，]+\.?\d*)",
        "扣非净利润": r"扣除非经常性损益的净利润[（(]?元?[)）]?\s*[|｜]?\s*(-?[\d,，]+\.?\d*)",
        
        # 每股指标
        "基本每股收益": r"基本每股收益[（(]?元/股[)）]?\s*[|｜]?\s*(-?[\d.]+)",
        "稀释每股收益": r"稀释每股收益[（(]?元/股[)）]?\s*[|｜]?\s*(-?[\d.]+)",
        
        # 资产负债表指标
        "总资产": r"总资产[（(]?元?[)）]?\s*[|｜]?\s*([\d,，]+\.?\d*)",
        "归属于上市公司股东的所有者权益": r"归属于上市公司股东的所有者权益[（(]?元?[)）]?\s*[|｜]?\s*([\d,，]+\.?\d*)",
        
        # 现金流指标
        "经营活动产生的现金流量净额": r"经营活动产生的现金流量净额[（(]?元?[)）]?\s*[|｜]?\s*(-?[\d,，]+\.?\d*)",
        
        # 收益率指标
        "加权平均净资产收益率": r"加权平均净资产收益率[（(]?%?[)）]?\s*[|｜]?\s*(-?[\d.]+)%?",
    }
